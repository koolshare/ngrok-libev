...
//https://github.com/zodiac1111/goahead/blob/master/balloc.c
#define IN_BALLOC
...
static bStatsType		bStats[B_MAX_CLASS] = {0};	/* Per class stats */
static bStatsFileType 	bStatsFiles[B_MAX_FILES] = {0};/* Per file stats */
static bStatsBlkType 	bStatsBlks[B_MAX_BLOCKS] = {0};/* Per block stats */
 ...
 *	See bfree.
 */
static bType			*bQhead[B_MAX_CLASS] = {0};	/* Per class block q head */
 ...
 */
	if (fp >= &bStatsFiles[bStatsFilesMax]) {
		for (fp = bStatsFiles; fp < &bStatsFiles[B_MAX_FILES]; fp++) {
			if (fp->file[0] == '\0') {
				gstrncpy(fp->file, name, FNAMESIZE-1);
				fp->allocated += size;
				fp->count++;
				fp->times++;
				fp->largest = size;
...
int main(int argc, char **argv) {
    ...
    if(0 != bopen(buf, 1024, 0)) {
        fprintf(stderr, "bopen error\n");
        return -1;
    }

    char* p = (char*)balloc(B_ARGS, 512);
    balloc(B_ARGS, 512);
    balloc(B_ARGS, 100);
    balloc(B_ARGS, 100);
